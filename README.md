...
Deploys run only in GitHub Actions, and `master` is the only source of what ships. Provisioning the server, the `production` environment's secrets and variables, the first `setup`, promoting `master` to `production`, hotfixes and day-to-day Kamal commands are in the [deploy runbook](docs/guides/kamal-deploy.md).

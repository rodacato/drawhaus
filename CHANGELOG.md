...
### Added
- **Prometheus metrics via `prom-client`.** The backend now exposes `GET /metrics` with `collectDefaultMetrics()` (process memory / GC / event-loop lag), an `http_request_duration_seconds` histogram labelled by `method` / matched-`route` / `status_code` (label cardinality bounded by using the route pattern, never the raw URL), and a `drawhaus_active_collaborators` gauge tracking live Socket.IO collaboration clients — the product's core value and main load driver. `/metrics` is served on a **dedicated port** (`METRICS_PORT`, default `9464`), separate from `PORT`, so kamal-proxy never routes it to the public hostname; a self-hosted Prometheus scrapes it over the private network / tailnet. Adds `prom-client@^15.1.3`.

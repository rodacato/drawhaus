...
### Changed
- **Frontend tests now run with coverage in CI** and upload `lcov.info` as a 14-day artifact (`ci.yml`). Step swapped from `npm run test` to `npm run test:coverage --workspace=frontend`; the report is available for download from each run and sets up the eventual `drawhaus-frontend` Sonar project.

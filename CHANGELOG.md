...
All notable changes to Drawhaus are documented here. The format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) from the next release on, and versions follow [Semantic Versioning](https://semver.org/); see [RELEASING.md](RELEASING.md).
...
- **An admin surface for webhooks** ([ADR-030](docs/adr/030-write-only-webhook-secrets.md)). Until now a webhook could only be created by calling the repository, which made the delivery machinery unreachable. Site Settings gains a Webhooks panel beside Integration Secrets: register an endpoint with a description and event checkboxes, pause or resume it, delete it, read its delivery log, and send a signed test event that answers while you are still looking at the screen. The API behind it is `GET`/`POST /api/admin/webhooks`, `PATCH`/`DELETE /api/admin/webhooks/:id`, `POST /api/admin/webhooks/:id/secret`, `GET /api/admin/webhooks/:id/deliveries` and `POST /api/admin/webhooks/:id/test`, all behind the existing admin guard with Zod on every input and `checkWebhookUrl` as the single URL validator, so a malformed URL, a non-`http(s)` scheme and embedded credentials each come back as a 400 naming the reason. The signing secret is generated server-side — the admin does not choose it — and returned exactly twice in its life, at creation and at regeneration; the serializer whitelists seven fields and a route test asserts that exact key set, so widening it fails loudly rather than quietly leaking a signing key. A `secret` smuggled into a `PATCH` body is stripped by the schema before it can reach the repository. The test event signs a `webhook.test` envelope and goes straight through the sender rather than the outbox, because the point of it is an answer now, not a delivery row in two minutes. Without `ENCRYPTION_KEY` the panel explains why webhooks are unavailable and the mutating endpoints answer 400, the same way integration secrets already degrade, instead of erroring or rendering an empty registry.

- **Webhooks for diagram events, delivered from a Postgres outbox** ([ADR-029](docs/adr/029-webhook-outbox.md)). An admin-registered endpoint is now notified of `diagram.created`, `diagram.updated`, `diagram.deleted`, `diagram.shared` and `template.created`. The five emission points were opened by this change — `CreateDiagramUseCase` and `DeleteDiagramUseCase` had no hook of any kind, and the spec's claim that they already existed was wrong, as was its suggestion to build on the `RealtimeNotifier` port, whose events are scene-shaped and share nothing with entity lifecycle. Each use case calls a fire-and-forget `WebhookDispatcher`, modelled on `AuditLogger.log`, after its write commits; a subscriber that is down, slow or returning 500 can never fail, slow or roll back the user's request. The dispatcher inserts one `webhook_deliveries` row per subscriber under a shared event id, and a 10-second `node-cron` poller drains the table, claiming rows with `SELECT ... FOR UPDATE SKIP LOCKED` — so the open question of whether production runs more than one backend instance does not apply here, and nothing depends on Redis, which is optional (ADR-021) and would have left the retry queue half-working on most self-hosted installs. A claim is a lease rather than a status change: it counts the attempt and pushes `next_attempt_at` out by 60s, so a process that dies mid-delivery releases its work instead of stranding a row. Three attempts with 30s/120s backoff, then the row is dead-lettered in place with its last error, which makes the outbox the failure log the spec asked for. Bodies are signed HMAC-SHA256 in Stripe's shape — `X-Drawhaus-Signature: t=<unix>,v1=<hex>` over `${t}.${body}`, so a captured body cannot be replayed — with a per-webhook secret stored AES-256-GCM encrypted under `ENCRYPTION_KEY`, the same mechanism integration secrets already use; without that key the feature is off. Payloads carry metadata only (never scene elements), and `diagram.shared` deliberately omits the share link's token, which is the capability to open the diagram. `diagram.updated` fires on API and MCP writes only: realtime canvas saves take the socket `save-scene` path and emit nothing, so a board edited all day produces no webhook traffic. On SSRF: non-`http(s)` schemes and URLs with credentials are refused, redirects are never followed, requests time out at 10s, and the response body is discarded unread so the admin-readable delivery log cannot become a way to read whatever the instance can reach; private and link-local addresses are deliberately still allowed, since calling a service on your own network is the point of a self-hosted webhook. Delivery is proven against a real local HTTP server that verifies the signature itself, and the repository against real Postgres, including that a row another transaction holds is skipped rather than waited on. No new HTTP endpoints — registration and the dead-letter view are a separate change.

- **Open boards are told about writes that did not come from their room** ([ADR-027](docs/adr/027-realtime-notifier-port.md)). A `RealtimeNotifier` port (`sceneReplaced`, `snapshotRestored`, `snapshotCreated`, `commentChanged`) with a single Socket.IO adapter replaces the mutable `IoHolder` that `snapshot.routes.ts` declared and `main.ts` filled. A content `PATCH` — REST save fallback, public API, MCP — now broadcasts `scene-from-db` to the diagram's room with the new elements, appState, scene id and the revision ADR-026 bumped it to, so the board updates without a reload instead of waiting for its next save to be refused. A title-only update notifies nothing: it never touches a scene (ADR-025), and the use case tests both halves. The REST comment routes (create, reply, resolve, delete) broadcast for the first time, matching the socket handlers' events, so a comment written while the socket is down is no longer invisible until a reload. Two notes carried in the ADR rather than fixed here: a client with unsaved local edits loses them silently when the broadcast lands (that is ADR-026's rule — merging is what resurrected deleted elements), and `POST /comments/:threadId/like` still reaches nobody, having no socket counterpart to mirror. The adapter is proven against a real Socket.IO server and client rather than a stubbed `io`.
...
- **Board callbacks no longer run from the render the socket listeners were bound in.** `eslint-plugin-react-hooks` now runs in the frontend lint (`rules-of-hooks` as an error, `exhaustive-deps` as a warning), and its 26 findings are fixed, with no suppressions. Three changes alter behaviour:
  - **Scene manager.** It bound `onConflict`, `onRemoteDelete` and `onEditsReplaced` once per socket generation, so a conflict, remote-delete or discarded-edits toast called whichever callback existed when the socket connected. They now go through `useEffectEvent` and always reach the latest render's callback, without re-subscribing listeners.
  - **Offline snapshot.** The same applies to its five-minute grace timer and its `beforeunload` save: both read the user and callbacks from when they fire, not from when the connection dropped.
  - **Socket connection.** `useSocketConnection` depended on `diagramId` while reading `joinMode`, which callers rebuild as an object literal every render. It now depends on the room id, share token and guest name themselves.

  The other findings were stable refs and the `SceneSync` instance added to dependency arrays, where nothing changes at runtime. Two new tests fail against the previous hooks: the scene manager and the offline snapshot each receive a callback swapped in after their listeners were bound.

- **A board tells you when a replace threw away your unsaved changes.** ADR-026 makes a scene from a new revision — a snapshot restore, an API or MCP content write — win over local edits computed before it, and ADR-027 made that land immediately instead of one debounce later. `useSceneManager` honoured the rule in silence: it short-circuited `localEdits` to `[]` before computing what it was discarding, so it could not even tell whether there had been anything to discard. It now computes the pending edit ids first and reports them, and the board raises a toast naming how many elements were lost. A replace that finds nothing unsaved — a plain restore, a reconnect, a `scene-from-db` on the same revision — still says nothing. The ids come from `SceneSync.editedIds`, not from the elements still on the canvas, so an element deleted here and not yet saved is reported too, although the replace brings it back rather than removing it. The discard itself is unchanged: merging is what resurrected the elements an API write had removed, which is the whole point of ADR-026. Nothing is stashed — the `drawhaus_scene_<id>` cache is the offline fallback the board reloads from, and writing discarded elements into it would put back exactly what the replace deleted.
- **A save the server refuses is unsaved again, so it is retried instead of sitting on `Error`.** Since ADR-028 the client knows a save failed, and could still do nothing about it: `persistScene` clears the unsaved marks before it emits — deliberately, because the save carries the scene as of emit time and edits made during the 5 s ack window are genuinely unsaved — so a refusal or a timeout left the content marked saved, `scheduleSave`'s `sync.hasUnsaved()` guard false, and the badge on `Error` until the user happened to draw something. `markSaved()` now returns the ids it cleared together with the baseline they belong to, and a failed save hands that back to `SceneSync.restoreUnsaved()`, which unions them into the unsaved set — marks added while the save was in flight are untouched. It is a no-op when a replace has reset the baseline in between, so a `stale` refusal (whose `scene-from-db` arrives alongside it) does not resurrect the edits ADR-026 just discarded. The failure also re-arms the debounce once, so a transient failure retries on its own; a refusal the server will never accept (`forbidden`, `not-in-room`) costs one extra attempt and then waits for the next real edit. Two saves can be in flight when the debounce fires during an ack wait, and each callback wrote `saveState` unconditionally, so a late `{ok:false}` from the first could overwrite the `saved` a newer save had already set; only the newest save now owns the badge.
...
- **Undo no longer reverts a teammate's change.** None of the `updateScene` calls that apply a collaborator's delta, the server's scene, a snapshot restore or a followed viewport passed `captureUpdate`, so Excalidraw used its default, `EVENTUALLY`, and folded them into the next local undo step: undoing your own rectangle also removed the one a teammate drew just before it. Every remote or system update now goes through `applyRemoteScene()` in the new `lib/excalidraw.ts` adapter, which passes `CaptureUpdateAction.NEVER`, the value Excalidraw documents for remote updates and scene initialization. The adapter types the canvas API as Excalidraw's own `ExcalidrawImperativeAPI` instead of the hand-written subset `lib/types.ts` declared.
- **An element you delete no longer comes back when a collaborator's older copy arrives.** Incoming deltas and full-scene updates were merged against `getSceneElements()`, which returns only live elements, so an element deleted here was missing from the local side of the merge and a stale remote copy was appended as if it were new. Merges now run against `getSceneElementsIncludingDeleted()`, where the deletion is a tombstone one version up that outranks any older copy, while a newer copy (a teammate's undo) still restores it. The unit-test stub returned deleted elements from `getSceneElements()`, unlike the real API, which is why no test caught it; it now mirrors the API.
- **Collaborators converge on the same element copies and z-order.** `mergeElements` and `mergeDelta` compared only `version` and kept the local copy on a tie, so two replicas holding different copies at the same version each kept their own forever. Both now apply Excalidraw's own `reconcileElements` rule through a shared `remoteWins()`: the higher version wins, and on equal versions the lower `versionNonce`. They also return elements in fractional-index order when every element has one, the order Excalidraw keeps its scene in, so z-order converges as well. A test replays four updates in all 24 orders, through full scenes and through deltas, and gets one result. `mergeDelta`'s orphan cleanup rewrote arrow bindings and group ids on the caller's element objects in place, without a version bump; it now returns new objects one version up (same nonce, so every replica produces the identical copy) and leaves its inputs untouched. The server-side merge in `updateSceneMerged` shares these helpers.
- **A drag's final position reaches collaborators, and a client no longer re-saves what it only received.** `useSaveManager` diffed each change against the element objects it had last sent, but Excalidraw mutates elements in place while dragging, so the kept "previous" copies moved along and the diff found nothing: in the E2E run a drag that ended at version 10 had broadcast only version 4. The baseline also started empty, so the first change of any kind made a client broadcast and re-save the whole scene it had just received. A `SceneSync` baseline now records, per element id, the version last sent or received and which local edits no save has carried yet; a change is whatever differs from it. The throttled broadcast reads the current scene when it fires, the save flushes it first and is skipped when nothing local is unsaved, and copies applied from the room are recorded as shared once applied. That also retires the echo-suppression counter that six call sites bumped around `updateScene` and released on a timer. `flushSave` (Ctrl+S) now saves deleted elements too, so a delete persists as the tombstone that outranks older copies.
- **The "someone modified an element you were editing" toast only fires when you were.** `mergeDelta` reported a conflict whenever the remote copy won and the local version was above zero, which is every real element, and the board toasted each one. A conflict now means a different remote copy replaced an element with local edits not yet saved: `mergeDelta` takes the edited ids from `SceneSync`. The remote-delete toast is limited the same way.
- **A save in flight during a restore no longer brings back what the restore removed.** A snapshot restore, and an API or MCP content `PATCH`, replace a scene, while `save-scene` merges by element version with no tombstones. A save computed before the replace and arriving after it therefore put the removed elements back on the server for good: every canvas showed the restore, and the next reload undid it. Scenes now carry a `revision` that only replaces bump. `scene-from-db` sends it, clients tag `save-scene`, `scene-delta` and `scene-update` with the revision their copy is based on, and the server checks it inside the same `SELECT ... FOR UPDATE` transaction that merges, so a save that waited on the replace's row lock is judged against what that replace committed. A stale save writes nothing and its sender gets the current scene back instead of `scene-saved`. Clients drop relayed deltas from an older revision; a scene from a new revision replaces their unsaved edits, while a plain reconnect keeps them. A payload without a revision is merged unchecked, so a tab open since before the deploy keeps saving until it reloads. This also narrows ADR-025's open end: an API write to an open board is no longer undone by that board's next save. See [ADR-026](docs/adr/026-scene-revisions.md).
- **Deleting an element a teammate is dragging now sticks.** ADR-022 promised "delete wins", but the code only honoured that for elements dropped from the array, never for the way Excalidraw actually deletes: marking the element and bumping its version. So a delete and a drag in flight at the same time just traded versions — the dragger's next frame outranked the tombstone and the element came back on both canvases, and on the server. `mergeDelta` now takes a remote tombstone over a local copy that has unsaved local edits, whatever the versions say, and bumps it past both so the deleter's replica settles on the delete instead of answering with its live copy. An undo that brings an element back is untouched: it is a live copy arriving over a local tombstone, which the version rule still decides. Elements nobody here was editing follow the version rule too.
...
- **Ctrl+S no longer reports a save the server never received, and a refused save no longer hangs the board or fakes a lost connection.** `save-scene` was fire-and-forget on both ends: the client emitted it and resolved `flushSave()` with `true` on the spot, so the "Diagrama guardado" toast fired before the frame reached the server, while four server-side paths returned in silence (socket not in the room, sender cannot edit, no scene id, and ADR-026's stale save) and left the badge on `Saving...` forever. A save that threw answered `room-error`, which the client treats as a failed connection, so one rejected write flipped the board into the connection-lost state — and since ADR-026 a stale or foreign `sceneId` surfaced that way on a perfectly connected board. `save-scene` now answers a Socket.IO acknowledgement on every path — `{ ok: true, sceneId }` once the write is committed, or `{ ok: false, reason }` with `not-in-room`, `forbidden`, `no-scene`, `stale` or `server-error` — sent before the interval snapshot and the Drive sync, which must not hold the save open. The shared `onEvent` wrapper forwards the ack callback when a client sends one, answers `invalid-payload` for a payload the schema rejects and `server-error` for a handler that throws, and answers at most once; handlers and clients that use no ack are untouched. The client emits with a 5 s timeout and reports `saved` only on `{ ok: true }`, so a refusal and a timeout both show `Error` and make `flushSave()` resolve `false` for the Ctrl+S toast and the leave-the-board guard. `scene-saved` stays as the compatibility path for a tab older than this change, and `room-error` is now emitted only by the room join handlers. The E2E test that drops every edit frame, presses Ctrl+S and asserts no saved toast is a real test again. See [ADR-028](docs/adr/028-save-acknowledgement.md).
- **Mermaid flowchart arrows are recognised with or without whitespace around them.** A `graph TD` whose body is `A-->B` imported nothing, and neither did any other unspaced arrow, labelled nodes included (`A[X]-->B`): the tokenizer looked for the next arrow with `search(/\s+(-{2,}|={2,}|-\.)/)`, which requires leading whitespace, so a line without it was pushed whole as one node token and rejected by `parseNodeDef`. The E2E fixme pinning this blamed unlabeled nodes; labels were never the variable. That search is now a character walk that stops at the first arrow outside any shape delimiter or quoted label, which also keeps `A[a--b]-->C` from splitting inside its own label. Node ids may contain hyphens (`order-service`), so a lone `-` stays part of the id while a run of two or more `-` or `=`, or `-.`, starts a link — the rule mermaid's own grammar applies (`A-B-->C` is `A-B --> C`; `A---B` is `A --- B`), checked case by case against mermaid 11's flowchart parser. Every edge style, chain and label form now parses the same spaced or unspaced.
- **Importing code that converts to nothing now reports it instead of silently adding nothing.** "Add to Canvas" closed the panel and left the board untouched whenever the conversion produced zero elements, which is exactly what the arrow bug above did. `convert-to-excalidraw.ts` now throws `EmptyDiagramError` when a conversion yields no elements, so both the Mermaid and the PlantUML path surface it — on import through the panel's error box, and for PlantUML already in the debounced preview, where it also keeps the import button disabled.
...
- **The MCP server is aligned on one zod, and `update_diagram` no longer reads a zod internal.** `packages/mcp` declared `zod@^3.24.4` while `@modelcontextprotocol/sdk@1.30.0` accepts `^3.25 || ^4.0` and resolved the hoisted `zod@4.6.2`, so the package built its schemas with a nested `zod@3.25.76` and handed them to an SDK running v4. `update-diagram.ts` derived the tool's input shape from `UpdateDiagramInput._def.schema.shape`, unwrapping the `ZodEffects` the "at least one field" refinement wraps around the object. `_def.schema` is a v3 internal and is gone in v4, so on the resolved SDK zod that access throws `Cannot read properties of undefined (reading 'shape')` — and it runs at registration time inside `registerAllTools`, which `main().catch()` turns into a fatal exit, taking down all six tools rather than one. The nested v3 install was the only thing keeping the server alive. `schemas.ts` now exports the base object as `UpdateDiagramFields` and applies the refinement on top, so the tool passes `UpdateDiagramFields.shape` like every other tool instead of reaching through internals, and the package declares `zod@^4.6.2`. The version split was also why `packages/*` was never type-checked: with two zod copies in the graph `tsc --noEmit` on `packages/mcp` exhausted a 1.4 GB heap, and it completes once the copies are unified. The root `typecheck` script now covers all four `packages/*` alongside the apps, and a new `tools.test.ts` registers every tool against a real `McpServer`, lists them through an in-memory SDK `Client`, and asserts `update_diagram` still rejects a call that changes nothing — restoring the `._def` access fails it.

- **Production reported version `0.0.0`.** `/api/version`, `/v1/health` and the webhook `User-Agent` read `npm_package_version`, which only npm sets, and the backend image starts `node dist/main.js` directly. The version now comes from the root `package.json`, which the image already copies; a config test boots the module without npm and fails on the old behaviour.

### Security

- **Anonymous clients can no longer open a socket** ([ADR-031](docs/adr/031-socket-handshake-auth.md)). There was no `io.use`: anyone who could reach `/socket.io/` got a connected socket, while `docs/ARCHITECTURE.md` described an auth middleware that did not exist. A handshake middleware now admits a connection only with a live session cookie for a user who is not disabled, or with an unexpired share token, which guests now send in Socket.IO's `auth` option instead of only after connecting. Anything else gets `connect_error` with `data.reason` `unauthenticated` (or `server-error` when the lookup fails). Socket.IO does not retry a refused socket, and the board shows why in es-MX rather than retrying: the session ended, the link is no longer valid, or access could not be verified. The handshake admits and nothing more: `join-room` and `join-room-guest` still resolve the credential and set the identity and room role, and the #157 membership checks are unchanged, so a socket admitted with a share link for one board gets nothing on another. A logout, an expired session or a revoked link did not close a board that was already open; its next reconnect was refused (closed on revoke since ADR-032). On the deploy that ships this, a signed-in tab reconnects normally, but a guest tab loaded before it has no token to send and is refused until the guest reloads; its badge says to reload.
- **Open sockets are now closed when their credential is revoked** ([ADR-032](docs/adr/032-close-sockets-on-revoke.md)). ADR-031 checked a credential only when a socket connected, so a board that was already open kept sending and receiving scene updates after the session or link behind it ended, for as long as the tab stayed open. Logout now closes the sockets of that session and leaves the same user's other tabs and devices connected. A password reset, an admin disabling or deleting the account, and the user deleting their own account close every socket of that user. Revoking a share link closes the sockets admitted with the link or that joined a board through it, and leaves the owner's signed-in socket and guests on other links connected. At the handshake each socket joins rooms named after its credential: a SHA-256 digest of the session or share token, never the token itself, because the Redis adapter sends room names to Redis. Each use case calls a new `accessRevoked` method on the `RealtimeNotifier` port after the credential is deleted, so a socket that reconnects in reply is refused. With the Redis adapter the close reaches every instance. The socket first receives `access-revoked` with a `reason`, and the board shows why in es-MX ("Tu sesión terminó…" or "Este enlace ya no es válido…") instead of "Reconnecting…". A socket layer that fails is logged and does not fail the logout or revoke. Expiry still closes nothing: an expired session or link keeps its open socket until the next reconnect, which is refused. The same is true for share links removed because their creator or their diagram was deleted.
- **Invalid socket payloads can no longer buy a log line each.** #157 answered a payload that fails its schema with a warning log and `event-error`, outside every per-socket limit, so one client could write the server's logs at the rate it could send frames. Rejected payloads now count against an `invalid` bucket of the existing per-socket limiter, 10 per second. Past it the rest of the window is dropped without `event-error` and without logging, except for one warning when the limit trips, so a flood still leaves a trace. The ack from ADR-028 is still answered `invalid-payload` for a dropped frame: it costs one packet per packet received and keeps a waiting client off its timeout. Valid events never count against the bucket. `docs/SOCKET_PROTOCOL.md` also claimed socket limits are off under `NODE_ENV=test`; they never were, and the doc now says so.
- **Text sanitization on the public API now runs in linear time.** `sanitizeElements` (applied to element `text` on `POST /v1/diagrams` and `PATCH /v1/diagrams/:id`) strips tags with a single bracket-matching scan. Its output is unchanged, and a test asserts that no tag survives sanitization.
- **Eleven of the twelve open Dependabot alerts closed by deduplicating `nanoid` and `lodash-es`.** All three packages were transitive and none could be reached by bumping a direct parent: `@excalidraw/excalidraw@0.18.1` and `@excalidraw/mermaid-to-excalidraw@2.2.2` are both the latest published releases and both pin `nanoid` to an exact version (`3.3.3` and `4.0.2`), and the vulnerable `lodash-es@4.17.21` copies are exact pins inside `chevrotain@11.0.3`, reached through `@mermaid-js/parser` → `langium@3.3.1`, whose own range is `~11.0.3`. Four scoped `overrides` entries therefore do the work: `nanoid` becomes `^3.3.19` under `@excalidraw/excalidraw` and `^5.1.16` under `@excalidraw/mermaid-to-excalidraw`, and `lodash-es` becomes `^4.18.1` tree-wide. The nanoid override is scoped rather than global on purpose — `postcss` needs the CommonJS 3.x line, and nanoid 5 is ESM-only, so a blanket override would have broken the Vite and Tailwind builds. The result removes four duplicate copies from the tree (`postcss` now shares the hoisted `nanoid@3.3.19`, and the three nested `lodash-es@4.17.21` copies under `chevrotain`, `@chevrotain/gast` and `@chevrotain/cst-dts-gen` collapse onto the `4.18.1` that `mermaid` and `chevrotain-allstar` already resolved to). Total frontend JavaScript moves by 18 bytes and the entry chunk is unchanged at 432.87 kB, because Vite was already tree-shaking and deduplicating these at bundle time. Worth recording alongside the fix: none of the eleven advisories was actually reachable here. The four `nanoid` ones all require attacker control of the `size` argument (non-integer, negative, or zero), and every call site in both packages passes either no argument or the literal `40`; the three `lodash-es` ones are in `_.template`, `_.unset` and `_.omit`, and the chevrotain family imports none of those three — it names only `includes`, `every`, `find`, `forEach`, `map`, `reduce` and similar core helpers. The versions were bumped anyway because the change is cheap, deduplicates the tree, and is safer than carrying eleven alerts whose triage has to be redone by hand every time the list is read.
- **The one remaining alert, `esbuild` (low, development-only), is left in place deliberately.** It is an arbitrary file read in esbuild's development server on Windows, fixed in `0.28.1`. The vulnerable `0.27.7` is pulled by `tsup@8.5.1` — the latest release, which pins `esbuild: ^0.27.0` — so clearing it would mean forcing `esbuild` outside its parent's declared range, which is the kind of override that fights the tree and has to be unwound later. Nothing in the repo invokes `esbuild --serve`; `tsup` uses the build API and Vite the transform API, and both the devcontainer and CI are Linux. It should be revisited when `tsup` ships support for `esbuild` 0.28.
...
- **The devcontainer can run the E2E suite and drops Docker-in-Docker.** `post-create.sh` waits for PostgreSQL, creates `drawhaus_e2e` if it is missing, and installs Chromium with its system libraries, all idempotent across rebuilds. PostgreSQL and Redis no longer publish host ports (`5432`, `6479`): the workspace reaches them over the compose network, and `forwardPorts` exposes PostgreSQL on a free local port. A fixed host port collides with any other project's container on the same machine, the likely reason the devcontainer's database was found not running. Docker-in-Docker was added so deploys could run from the container. Deploys run in CI now, and the local Kamal commands only read over SSH, so a privileged container no longer pays for itself; image builds moved to CI (above).
- **Ruby 4.0 for Kamal**, in the devcontainer and the deploy workflow. Ruby 3.2 reached end of life in April 2026. Kamal 2.12 installs on Ruby 4.0.6 with its native extensions (`ed25519`, `bcrypt_pbkdf`), and both deploy configs render.
...
- **Public hostnames come from two required variables, `APP_HOST` and `API_HOST`.** The Kamal proxy hosts were hardcoded to the maintainer's domain, and five more variables had to agree with them by hand. Now `config/deploy.frontend.yml` and `config/deploy.backend.yml` take their proxy hosts from them and derive `FRONTEND_URL`, `VITE_API_URL`, `VITE_WS_URL` and both OAuth redirect URIs, so those five are no longer read from the `production` environment. Nothing defaults: an empty host would make kamal-proxy route every hostname, so the workflow stops before Kamal, the Kamal ERB refuses anything but a bare hostname, and the backend refuses to boot in production unless `FRONTEND_URL` is an absolute `http(s)` URL with a host (`"https://"` used to pass). The mail sender falls back to `noreply@` + that host instead of `noreply@drawhaus.app`; an explicit `FROM_EMAIL` still wins.

- **The deploy workflow no longer depends on an external shared action.** `build-push.yml` (now named "Deploy") runs the steps the shared `kamal-deploy` action ran — Ruby 3.2, `gem install kamal -v "~> 2.7"`, Buildx, the SSH agent, `known_hosts`, accessory boot for the backend and the Kamal command — directly, with the same pinned actions. The Docker Hub login stays skipped when `DOCKERHUB_USERNAME` is unset, as it was inside the action.

- **The quality workflow no longer depends on an external shared workflow.** `quality.yml` runs Semgrep, Trivy and Gitleaks as its own jobs, with the versions the shared workflow pinned (`semgrep/semgrep`, `trivy-action@v0.36.0`, `gitleaks-action@v3`) and still report-only, now behind one `SECURITY_BLOCKING` env. The Sonar job calls `sonarqube-scan-action@v8.2.0` directly and runs only on dispatch **and** when the `SONAR_HOST_URL` variable is set, so a fork without SonarQube skips it. It also builds `@drawhaus/mcp` before the suites, as `ci.yml` does.
...
- **Each production image is built once, by `kamal deploy`.** The separate build-push jobs pushed `<sha>` and `latest`, and `kamal deploy` then rebuilt and pushed the same tags, so production already ran Kamal's build, which never received `GIT_COMMIT`; the backend config now passes it. Kamal is pinned to 2.12.0 instead of `~> 2.7`. Both configs drop `builder.context: .`, so Kamal builds from a clone of the commit instead of the working tree. The runbook's migration command pointed at a script that does not exist (migrations run on boot) and its backup command needed `tsx`, which the image does not install; both are corrected.
...
- **The release process moves to `RELEASING.md`.** A release is an annotated tag on a merged commit plus a GitHub Release built from its changelog section, and it no longer deploys: promoting `master` to `production` is a separate step in the deploy runbook. The old guide pushed `master --tags`, merged hotfix branches straight into `production`, and bumped three `package.json` files by hand; one `npm version` command now moves the root, backend and frontend together. The changelog's pending section is `## [Unreleased]`, per Keep a Changelog.

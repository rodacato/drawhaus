...
| **Webhooks**        | HMAC-SHA256 signed delivery of `diagram.created` / `.updated` / `.deleted` / `.shared` and `template.created` to admin-registered endpoints, Postgres outbox with 3 attempts and exponential backoff, dead-letter log ([ADR-029](adr/029-webhook-outbox.md)), admin registration UI with write-only secrets, delivery log and a send-test action ([ADR-030](adr/030-write-only-webhook-secrets.md)) | v0.12 |
| **Security**        | Helmet headers, rate limiting (Redis-backed when available), audit logger, RBAC, cookie hardening, encrypted secrets in DB, socket handshake auth, open sockets closed when their session or share link is revoked                                                                                                                                                                                  | v0.8  |
...
| **Testing**         | Playwright E2E suite in CI (permissions, CRUD, sharing, auth, visual regression, real-canvas collaboration), backend unit + integration tests (`node --test`), frontend tests (Vitest + jsdom), helpers tests, marketing screenshot automation                                                                                                                                                      | v0.8  |
...
| [webhooks](specs/webhooks.md)                         | Webhooks                             | shipped                                                          |
...
| Closing sockets on revoke          | Access rooms per credential (hashed); `accessRevoked` on the notifier | Logout, disable and link revoke left open tabs editing; rooms reach every instance through Redis    | [032](adr/032-close-sockets-on-revoke.md)            |

    ...
    <div className="pointer-events-auto flex items-center gap-1 rounded-lg bg-white px-2 py-1 shadow-sm">
      {scenes.map((scene) => (
        <div key={scene.id} className="group relative flex items-center">
          ...
          {canEdit && scenes.length > 1 && scene.id === activeSceneId && editingId !== scene.id && (
            <button
              className="ml-0.5 hidden rounded p-0.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 group-hover:block"

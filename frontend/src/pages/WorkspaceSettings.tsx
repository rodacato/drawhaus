...
export function WorkspaceSettings() {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await workspacesApi.update(id, { name: name.trim(), description: description.trim(), color, icon });
  ...
  async function handleRoleChange(userId: string, newRole: string) {
    if (!id) return;
    try {
      await workspacesApi.updateMemberRole(id, userId, newRole);
      loadData();
    } catch { /* silent */ }
  }
    ...
    <div className="min-h-screen bg-surface">
      <div className="mx-auto max-w-3xl px-6 py-12">
        ...
        <div className="mb-8 flex items-center gap-4">
          <Link to="/dashboard" className="text-text-muted hover:text-text-primary transition">
          ...
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg text-lg" style={{ backgroundColor: workspace.color + "20", color: workspace.color }}>
              {workspace.icon || workspace.name.charAt(0).toUpperCase()}
            </div>
            <div>
              <h1 className="font-[family-name:var(--font-family-heading)] text-2xl font-bold text-text-primary">Workspace Settings</h1>

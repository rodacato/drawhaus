...
test.describe("User Registration", () => {
  ...
  test("POST /api/auth/register creates a new user", async ({ request, baseURL }) => {
    ...
    const adminCtx = await loginAsUser(baseURL!, ADMIN_USER.email, ADMIN_USER.password);
    ...
    const tempUser = (Array.isArray(users) ? users : []).find((u: any) => u.email === uniqueEmail);
    expect(tempUser).toBeTruthy();

    // Delete the temp user
    const deleteResponse = await adminCtx.delete(`/api/admin/users/${tempUser.id}`);

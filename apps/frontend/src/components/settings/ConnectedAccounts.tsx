...
export function ConnectedAccounts() {
  ...
  useEffect(() => {
    ...
    if (linked) {
      setLinkStatus({
        type: "success",
        message: `${linked === "google" ? "Google" : "GitHub"} account linked successfully`,
      });
      refreshUser();
      ...
      setLinkStatus({
        type: "error",
        message: `Failed to link ${linkError === "google" ? "Google" : "GitHub"} account`,
      });
      const params = new URLSearchParams(searchParams);
  ...
  async function handleUnlinkProvider(provider: Provider) {
    ...
    try {
      ...
      setLinkStatus({
        type: "success",
        message: `${provider === "google" ? "Google" : "GitHub"} account disconnected`,
      });
      refreshUser();
    } catch (err: unknown) {
  ...
  const lastMethodWarning = isLastSignInMethod
  ...
  return (
    <div className={ui.card}>
      ...
      {linkStatus && (
        <p className={`mt-3 ${linkStatus.type === "error" ? ui.alertError : ui.alertSuccess}`}>
          {linkStatus.message}
        </p>
      )}
      <div className="mt-4 space-y-3">
        ...
        <div className="flex items-center justify-between rounded-lg border border-border p-4">
          ...
          {user?.linkedProviders?.includes("google") ? (
            ...
            <a
              href={`${API_URL}/api/auth/link/google`}
              className={`${ui.btn} ${ui.btnSecondary} text-xs`}
            >
              Connect
            </a>
        ...
        <div className="flex items-center justify-between rounded-lg border border-border p-4">
          ...
          {user?.linkedProviders?.includes("github") ? (
            ...
            <a
              href={`${API_URL}/api/auth/link/github`}
              className={`${ui.btn} ${ui.btnSecondary} text-xs`}
            >
              Connect
            </a>

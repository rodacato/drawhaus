...
export function TemplatePicker({
  ...
  async function handleUseCustom(template: TemplateDTO) {
    ...
    setPending(true);
    try {
      await onUseTemplate(template.id, template.title);
    } finally {
      setPending(false);
    }
  }
    ...
    return (
      <div className="group relative flex min-h-[140px] flex-col rounded-xl border border-border bg-surface text-left transition hover:border-primary/30 hover:shadow-md">
        ...
        >
          ...
          <span className="mt-2 text-sm font-semibold text-text-primary group-hover:text-primary">
            {template.title}
          </span>
          <span className="mt-1 text-xs text-text-muted line-clamp-2">
            {template.description || "Custom template"}
          </span>
          <span className="mt-auto flex items-center gap-2 pt-2">
            <span className="inline-block rounded-full bg-surface-raised px-2 py-0.5 text-[10px] font-medium text-text-muted">
              {template.category}
            </span>
  ...
  const hasResults =
    filteredBuiltIn.length > 0 || filteredPersonal.length > 0 || filteredWorkspace.length > 0;
      ...
      <div className="relative z-10 flex max-h-[80vh] w-full max-w-3xl flex-col rounded-2xl border border-border bg-surface-raised shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-2 text-text-muted transition hover:bg-surface hover:text-text-primary"
          >
            <svg
              width="18"
              height="18"
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              ...
              {filteredBuiltIn.map((template) => (
                ...
                >
                  <span className="text-2xl text-primary/70">

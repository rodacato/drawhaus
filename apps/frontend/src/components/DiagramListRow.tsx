...
export function DiagramListRow({
  ...
  return (
    <div className="group flex items-center gap-4 px-4 py-3 transition hover:bg-surface-raised">
      <Link
        to={`/board/${diagram.id}`}
        className="block h-12 w-16 shrink-0 overflow-hidden rounded-md border border-border bg-surface"
      >
        {diagram.thumbnail ? (
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2">
          {diagram.tags && diagram.tags.length > 0 && <TagBadges tags={diagram.tags} />}
          <p className="text-xs text-text-muted">
            {new Date(diagram.updatedAt ?? diagram.updated_at ?? "").toLocaleDateString(undefined, {
              year: "numeric",
              month: "short",
              day: "numeric",
              hour: "2-digit",
      ...
      <div className="flex shrink-0 items-center gap-1 opacity-0 transition group-hover:opacity-100">
        ...
        <button
          onClick={() => onDelete(diagram.id, diagram.title)}

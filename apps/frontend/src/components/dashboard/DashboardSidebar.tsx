    ...
    <aside className="flex w-72 shrink-0 flex-col justify-between border-r border-border bg-surface-raised">
      <div className="p-6">
        ...
        <Link to="/dashboard" className="mb-10 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-[family-name:var(--font-family-heading)] text-xl font-bold tracking-tight text-text-primary">
              Drawhaus
            </h1>
        ...
        <nav className="space-y-1">
          ...
          {/* Workspaces section */}
          <div className="pb-2 pt-4 px-3 text-[10px] font-bold uppercase tracking-wider text-text-muted">
            Workspaces
          </div>
          {workspaces.map((ws) => (
            <div key={ws.id} className="group flex items-center">
              <button
                ...
                className={navBtnClass(
                  activeWorkspaceId === ws.id && !isRecent && !isStarred && !isTemplates,
                )}
                type="button"
              >
                <span
            ...
            <button
              onClick={() => {
                workspacesApi
                  ...
                  .catch(() => {
                    onStatusMessage("Could not create workspace.");
                  });
              }}
        ...
        <div className="border-t border-border p-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex shrink-0 items-center gap-1">
              <Link
                to="/settings"
                className="flex h-8 w-8 items-center justify-center rounded-lg text-text-muted transition hover:bg-surface hover:text-primary"
                title="Settings"
              >
                <svg
                  width="16"

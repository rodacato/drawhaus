...
describe("commentsApi", () => {
  ...
  test("list omits params when sceneId not provided", async () => {
    ...
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/diagrams/d1/comments", { params: undefined }]);
  });

  test("list includes sceneId when provided", async () => {
    const stub = mock.method(api, "get", () => Promise.resolve([]));
    await commentsApi.list("d1", "s1");
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/diagrams/d1/comments", { params: { sceneId: "s1" } }]);
  });
  ...
  test("delete removes thread", async () => {
    ...
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/diagrams/d1/comments/t1"]);
  });

  test("toggleLike posts to /like and returns liked+count", async () => {
    const stub = mock.method(api, "post", () => Promise.resolve({ liked: true, likeCount: 3 }));
    const result = await commentsApi.toggleLike("d1", "t1");
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/diagrams/d1/comments/t1/like"]);

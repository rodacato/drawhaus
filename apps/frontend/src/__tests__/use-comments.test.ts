...
function render(
  opts: { socket?: MockSocket | null; diagramId?: string; sceneId?: string | null } = {},
) {
...
describe("useComments", () => {
  ...
  test("loads threads on mount with the current diagram and scene", async () => {
    const listSpy = vi
      .spyOn(commentsApi, "list")
      .mockResolvedValue({ threads: [makeThread()] } as never);
  ...
  test("socket comment-created adds a thread for the current diagram", async () => {
    ...
    act(() =>
      triggerSocketEvent(socket, "comment-created", {
        roomId: "d1",
        thread: makeThread({ id: "new" }),
      }),
    );
  ...
  test("socket comment-created ignores threads from another diagram", async () => {
    ...
    act(() =>
      triggerSocketEvent(socket, "comment-created", {
        roomId: "x",
        thread: makeThread({ id: "new", diagramId: "other" }),
      }),
    );
    expect(result.current.threads).toEqual([]);
  });
  ...
  test("socket comment-created filters threads from a different scene but keeps general ones", async () => {
    ...
    act(() =>
      triggerSocketEvent(socket, "comment-created", {
        roomId: "d1",
        thread: makeThread({ id: "other-scene", sceneId: "s2" }),
      }),
    );
    expect(result.current.threads).toEqual([]);
    ...
    act(() =>
      triggerSocketEvent(socket, "comment-created", {
        roomId: "d1",
        thread: makeThread({ id: "same-scene", sceneId: "s1" }),
      }),
    );
    act(() =>
      triggerSocketEvent(socket, "comment-created", {
        roomId: "d1",
        thread: makeThread({ id: "general", sceneId: null }),
      }),
    );
  ...
  test("socket comment-replied appends a reply and dedups by reply id", async () => {
    ...
    act(() =>
      triggerSocketEvent(socket, "comment-replied", {
        roomId: "d1",
        threadId: "th1",
        reply: makeReply({ id: "r1" }),
      }),
    );
    act(() =>
      triggerSocketEvent(socket, "comment-replied", {
  ...
  test("socket comment-resolved patches the resolution fields", async () => {
    ...
    act(() =>
      triggerSocketEvent(socket, "comment-resolved", {
        roomId: "d1",
        thread: makeThread({
          id: "th1",
          resolved: true,
  ...
  test("socket comment-deleted removes the thread", async () => {
    ...
    const { result } = await renderLoaded({
      socket,
      seed: [makeThread({ id: "th1" }), makeThread({ id: "th2" })],
    });
  ...
  test("createThread emits over the socket and skips the REST call when connected", async () => {
    ...
    await act(async () => {
      await result.current.createThread("el9", "hello");
    });
    expect(socket.emit).toHaveBeenCalledWith("comment-create", {
      roomId: "d1",
      elementId: "el9",
      body: "hello",
      sceneId: "s1",
  ...
  test("createThread falls back to REST and appends the thread when no socket", async () => {
    const createSpy = vi
      .spyOn(commentsApi, "create")
      .mockResolvedValue({ thread: makeThread({ id: "rest" }) } as never);
    const { result } = await renderLoaded({ socket: null });

    await act(async () => {
      await result.current.createThread("el1", "hi");
    });
  ...
  test("addReply falls back to REST and appends the reply when no socket", async () => {
    vi.spyOn(commentsApi, "list").mockResolvedValue({
      threads: [makeThread({ id: "th1" })],
    } as never);
    const replySpy = vi
      .spyOn(commentsApi, "reply")
      .mockResolvedValue({ reply: makeReply({ id: "r9" }) } as never);
    const { result } = render({ socket: null });
    await waitFor(() => expect(result.current.loading).toBe(false));

    await act(async () => {
      await result.current.addReply("th1", "yo");
    });
  ...
  test("resolveThread falls back to REST and patches resolved state when no socket", async () => {
    ...
    await act(async () => {
      await result.current.resolveThread("th1", true);
    });
  ...
  test("deleteThread falls back to REST and removes the thread when no socket", async () => {
    ...
    await act(async () => {
      await result.current.deleteThread("th1");
    });
  ...
  test("toggleLike always calls REST and patches the like state", async () => {
    vi.spyOn(commentsApi, "list").mockResolvedValue({
      threads: [makeThread({ id: "th1", likeCount: 0, likedByMe: false })],
    } as never);
    const likeSpy = vi
      .spyOn(commentsApi, "toggleLike")
      .mockResolvedValue({ liked: true, likeCount: 3 } as never);
    const socket = createMockSocket();
    const { result } = render({ socket });
    await waitFor(() => expect(result.current.loading).toBe(false));

    await act(async () => {
      await result.current.toggleLike("th1");
    });

...
describe("workspacesApi", () => {
  ...
  test("list calls /api/workspaces", async () => {
    const stub = mock.method(api, "get", () => Promise.resolve({ workspaces: [] }));
    await workspacesApi.list();
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/workspaces"]);
  });

  test("get fetches workspace by id", async () => {
    const stub = mock.method(api, "get", () => Promise.resolve({ workspace: {}, role: "admin", members: [] }));
    await workspacesApi.get("w1");
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/workspaces/w1"]);
  });

  test("create posts workspace data", async () => {
    const stub = mock.method(api, "post", () => Promise.resolve({ workspace: {} }));
  ...
  test("update patches workspace fields", async () => {
    const stub = mock.method(api, "patch", () => Promise.resolve({ workspace: {} }));
    await workspacesApi.update("w1", { name: "renamed" });
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/workspaces/w1", { name: "renamed" }]);
  });
  ...
  test("removeMember deletes member", async () => {
    ...
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/workspaces/w1/members/u1"]);
  });

  test("acceptInvite posts token", async () => {
    const stub = mock.method(api, "post", () => Promise.resolve({}));
    await workspacesApi.acceptInvite("tok");
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/workspaces/accept-invite", { token: "tok" }]);
  });

  test("resolveInvite fetches invite by token", async () => {
    const stub = mock.method(api, "get", () => Promise.resolve({ workspaceName: "T", role: "editor", email: "a@b.com" }));
    await workspacesApi.resolveInvite("tok");
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/workspaces/invite/tok"]);
  });

  test("transferOwnership posts newOwnerId+transferResources", async () => {
    const stub = mock.method(api, "post", () => Promise.resolve({ success: true, diagramCount: 0, templateCount: 0 }));
  ...
  test("listOwnedShared calls /api/workspaces/owned-shared", async () => {
    ...
    assert.deepEqual(stub.mock.calls[0].arguments, ["/api/workspaces/owned-shared"]);
  });
});

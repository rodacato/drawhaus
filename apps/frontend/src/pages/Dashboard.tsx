...
export function Dashboard() {
  ...
  const [viewMode, setViewMode] = useState<"grid" | "list">(
    () => (localStorage.getItem("drawhaus_view") as "grid" | "list") ?? "grid",
  );
  ...
  useEffect(() => {
    localStorage.setItem("drawhaus_view", viewMode);
  }, [viewMode]);
  ...
  function navTo(fId?: string | null) {
    if (fId === undefined) {
      ...
      setSearchParams({ folderId: "null" });
      setSidebarView("unfiled");
    } else {
    ...
    <div className="flex h-screen overflow-hidden bg-surface">
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <div className="mb-6">
            <h2 className="font-[family-name:var(--font-family-heading)] text-3xl font-bold tracking-tight text-text-primary">
              {data.heading}
            </h2>
        ...
        icon={
          settingsWorkspaceId ? (
            <div
              ...
              style={{
                backgroundColor:
                  (data.workspaces.find((w) => w.id === settingsWorkspaceId)?.color ?? "#6366f1") +
                  "20",
                color:
                  data.workspaces.find((w) => w.id === settingsWorkspaceId)?.color ?? "#6366f1",
              }}
            >
              {data.workspaces.find((w) => w.id === settingsWorkspaceId)?.icon ||
                data.workspaces
                  .find((w) => w.id === settingsWorkspaceId)
                  ?.name.charAt(0)
                  .toUpperCase()}
            </div>
          ) : undefined

...
describe("startBackupScheduler — disabled config", () => {
  it("does not schedule when BACKUP_ENABLED=false and logs the 'disabled' message", async () => {
    ...
    assert.equal(
      ...
      0,
      "cron.schedule must not be called when disabled",
    );
...
describe("startBackupScheduler — invalid cron expression", () => {
  it("does not schedule and logs an error when cron.validate returns false", async () => {
    ...
    assert.equal(
      scheduleMock.mock.calls.length,
      0,
      "cron.schedule must not be called when expression is invalid",
    );
    ...
    assert.equal(
      errorMock.mock.calls[0].arguments[1],
      "Invalid backup cron expression, scheduler not started",
    );
  });
});
